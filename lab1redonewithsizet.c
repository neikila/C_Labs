...
void OutputTest( size_t* IntMas, size_t size )
{
	printf("%d\n%d", (int)size, (int)*IntMas );
	size_t i = 1;
	for (; i < size; ++i )
		printf(" %d", (int)(IntMas[i]) );
	printf("\n");
}
...
size_t* input( size_t* size )
{
	if( scanf("%d", (int*)size ) == 1 )
	{
		if ( *size >= 0 || getchar() == '\n' )
		{
			size_t* IntMas = (size_t*) malloc( sizeof(size_t) * (*size) );
			if ( IntMas != NULL )
			{
				size_t temp, sym, i = 0;
				for (; i < *size; ++i)
...
void SearchSequence( size_t* IntMas, size_t Size, size_t* MIndex, size_t* MSize )
{
	size_t i = 0;
...
int main()
{
	...
	size_t* IntMas = input(&N);//Входные данные
	if ( IntMas == NULL )
	{
		printf("[error]\n");
		return 0;
